...
(function(){
 ...
 window.createHouseCity=function(THREE){
  ...
  const buildings=new THREE.InstancedMesh(geo(1,1,1),mat(0xffffff,{vertexColors:true}),BUILDINGS.length);
  BUILDINGS.forEach((b,i)=>{matrix.compose(new THREE.Vector3(b.x,b.h/2,b.z),new THREE.Quaternion(),new THREE.Vector3(b.w,b.h,b.d));buildings.setMatrixAt(i,matrix);buildings.setColorAt(i,color.setHex(b.c));collisions.push({minX:b.x-b.w/2,maxX:b.x+b.w/2,minZ:b.z-b.d/2,maxZ:b.z+b.d/2,id:`city-building-${i+1}`})});
  buildings.instanceMatrix.needsUpdate=true;buildings.instanceColor.needsUpdate=true;buildings.computeBoundingBox();buildings.computeBoundingSphere();root.add(buildings);
  ...
  function update(){}
  function debug(){return {size:CONFIG.size,buildingCount:BUILDINGS.length,windowInstances:transforms.length,lampInstances:lampPositions.length,collisionCount:collisions.length,budgets:{...CONFIG.budgets},estimatedModuleDrawCalls:10,estimatedModuleTriangles:BUILDINGS.length*12+transforms.length*12+lampPositions.length*24+dashCount*12+36}}
  function dispose(){root.removeFromParent();resources.forEach(v=>v.dispose());resources.clear()}
  return {group:root,root,config:CONFIG,collisions,exteriorSpawn:{x:0,z:9.5},canWalk,update,debug,dispose,destroy:dispose};
 };
})();
