...
(function(root,factory){
 ...
 function create(THREE){
  ...
  world.ready=Promise.all([loadLevel(),loadAssets()]).then(([loadedLevel,assets])=>{
   ...
   const floorSize=level.width/2,floorGeometry=new THREE.BoxGeometry(floorSize,.1,floorSize),floorMaterial=new THREE.MeshBasicMaterial({color:0xc7c6bd});
   for(const x of [-floorSize/2,floorSize/2])for(const z of [-floorSize/2,floorSize/2]){const floor=new THREE.Mesh(floorGeometry,floorMaterial);floor.position.set(x,-.07,z);floor.frustumCulled=false;floor.receiveShadow=false;floor.userData.assetId="city.sidewalk.base";group.add(floor)}
