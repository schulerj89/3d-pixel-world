...
(function(root,factory){
 ...
 const ASSET_REGISTRY=Object.freeze({
  T:{assetId:"restaurant.table.round",color:0xb87955,size:[1.35,.78,1.35],height:.39},
  C:{assetId:"restaurant.chair.dining",color:0xd79a78,size:[.72,1.05,.72],height:.525},
  B:{assetId:"restaurant.booth.single",color:0xc75f87,size:[.9,1.15,1.5],height:.575},
  H:{assetId:"restaurant.host.stand",color:0x8f5d45,size:[.9,1.25,.7],height:.625},
  P:{assetId:"restaurant.plant.potted",color:0x5da56c,size:[.75,1.35,.75],height:.675},
  K:{assetId:"restaurant.kitchen.prep",color:0xb9c6cf,size:[.92,1,.92],height:.5},
  S:{assetId:"restaurant.kitchen.stove",color:0x555d66,size:[.92,1,.92],height:.5},
  F:{assetId:"restaurant.kitchen.fridge",color:0xdce8ed,size:[.92,2.2,.92],height:1.1},
  W:{assetId:"restaurant.kitchen.sink",color:0x76aab8,size:[.92,1,.92],height:.5},
  R:{assetId:"restaurant.kitchen.rack",color:0x9b765a,size:[.92,1.8,.92],height:.9}
 });
 ...
 function buildRuntime(THREE,scene,rooms){
  ...
  for(const [symbol,spec] of Object.entries(ASSET_REGISTRY)){
   const placements=[];rooms.forEach(room=>room.map.forEach((row,r)=>[...row].forEach((value,c)=>{if(value===symbol)placements.push(cellCenter(room,c,r))})));
  ...
  group.userData={destination:"restaurant",rooms:rooms.map(room=>({id:room.room,width:room.width,depth:room.depth,layoutFile:ROOM_FILES[room.room]})),assetRegistry:ASSET_REGISTRY,npcs:0,orders:false,hud:false};
  scene.add(group);
  return {group,rooms,spawn:cellCenter(rooms[0],rooms[0].spawnCol,rooms[0].spawnRow),camera:{angle:.35,height:11,distance:15},canWalk:(x,z)=>canWalk(rooms,x,z)};
 }
 ...
 async function ensure(THREE,scene,fetchImpl){
  ...
  loading=loadRooms(fetchImpl).then(rooms=>runtime=buildRuntime(THREE,scene,rooms)).finally(()=>loading=null);return loading;
 }
 ...
 return {ROOM_FILES,ASSET_REGISTRY,WALKABLE,parseLevel,cellCenter,roomAt,symbolAtWorld,canWalk,doorwayCells,validateConnection,buildRuntime,loadRooms,ensure,destroy,get current(){return runtime}};
});
